...
FAN_MANUAL_MODE_PATHS = [
    ...
    "/sys/class/hwmon/hwmon5/pwm5_enable",
...
MAXTEMP = 31  # Temperature at which the fan reaches maximum speed (in °C)
MINSTART = 100  # Minimum PWM value at which the fan starts spinning
MINSTOP = 20  # PWM value at which the fan stops spinning

# Function to set the PWM value (0 to 255) using sudo
def set_fan_speed(pwm_path, speed):
    try:
        command = f"echo {speed} | sudo tee {pwm_path} > /dev/null"
...
def set_manual_mode(fan_manual_mode_path, mode):
    try:
        command = f"echo {mode} | sudo tee {fan_manual_mode_path} > /dev/null"
        subprocess.run(command, shell=True, check=True)
